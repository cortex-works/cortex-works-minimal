                ...
                "tools": [
                    {
                        ...
                        "description": "Map an unfamiliar repo, inspect workspace topology, deep-slice relevant files, or emit a signatures-only skeleton. Start with workspace_topology or map_overview for orientation; use deep_slice when you need bodies and context. You can operate on multiple workspace roots simultaneously. Provide arrays of target directories (e.g. target_dirs=['[ProjectA]','[ProjectB]']) to analyze or edit cross-repo features.",
                        "inputSchema": {
                            "type": "object",
                            "properties": {
                                "action": {
                                    ...
                                    "description": "workspace_topology: ultra-low-token workspace summary listing only discovered projects, manifests, and language hints. map_overview: bird's-eye symbol map of one or more dirs; in multi-root mode prefer target_dirs=['[Host]','[Daemon]'] instead of '.'. deep_slice: token-budgeted XML with bodies (requires target file/dir; use single_file=true for a specific file, query for semantic ranking). skeleton: project-wide YAML signatures-only constitution for one or more explicit target dirs."
                                },
                                "repoPath": { "type": "string", "description": "Abs path to the primary repo root. Omit when using [FolderName]/ prefixed paths (multi-root workspace)." },
                                "target_project": { "type": "string", "description": "Cross-project: ID or abs path from network map. Overrides repoPath." },
                                "target_dirs": { "type": "array", "items": { "type": "string" }, "description": "(map_overview, skeleton) One or more dirs to analyze. In multi-root workspaces, prefer explicit prefixes such as ['[Host]','[Daemon]'] or ['[Host]/src','[Daemon]/src'] to conserve tokens and analyze cross-repo features simultaneously." },
                                ...
                                "target": { "type": "string", "description": "(deep_slice) Relative path to file or dir. In multi-root workspaces prefix with [FolderName]/, e.g. '[AnvilSynth]/src/main.rs'." },
                                ...
                                "only_dirs": { "type": "array", "items": { "type": "string" }, "description": "(deep_slice) Restrict semantic search to one or more dirs, e.g. ['[Backend]','[Frontend]']. You can operate on multiple workspace roots simultaneously; use arrays to scope cross-repo feature work without indexing the whole workspace." },
                    ...
                    },
                    {
                        ...
                        "description": "Exact symbol analysis for reading source, finding usages, implementations, blast radius, and propagation checklist. Use this when you already know the symbol or need precise impact analysis.",
                        "inputSchema": {
                            "type": "object",
                            "properties": {
                                "action": {
                                ...
                                "target_dir": { "type": "string", "description": "Scope dir ('.' = whole repo). In multi-root workspaces prefix with [FolderName]/ to target a specific root, e.g. '[Host]/src'. Required for find_usages/blast_radius." },
                                "ignore_gitignore": { "type": "boolean", "description": "(propagation_checklist) Include git-ignored files." },
                                "max_chars": { "type": "integer", "description": "Max output chars. Default 8000." },
                                "only_dir": { "type": "string", "description": "(propagation_checklist) Restrict scan to this subdir. Accepts [FolderName]/ prefix for multi-root." },
                                "aliases": { "type": "array", "items": { "type": "string" }, "description": "(propagation_checklist) Alternative names across language boundaries." },
                                "path": { "type": "string", "description": "(read_source) Source file. Accepts [FolderName]/path/to/file for multi-root workspaces. Required." },
                    ...
                    {
                        "name": "cortex_chronos",
                        "description": "AST-aware checkpointing for risky refactors. Save a checkpoint before editing and compare against live code afterward without being distracted by formatting-only diffs.",
                        "inputSchema": {
                            "type": "object",
                            "properties": {
                                "action": {
                                ...
                                "max_chars": { "type": "integer", "description": "Max output chars. Default 8000." },
                                "path": { "type": "string", "description": "Source file. Accepts [FolderName]/path/to/file for multi-root workspaces. Required for save; optional for compare." },
                    ...
                    // cortex_run_diagnostics removed from public tool list
                    // (merged into cortex_act_shell_exec run_diagnostics=true)
                    // Kept as unlisted compat shim in tool_call dispatch below.

                    // cortex_manage_ast_languages kept active

                    {
                        "name": "cortex_manage_ast_languages",
                        "description": "Download and hot-reload extra Tree-sitter Wasm grammars from GitHub releases. Core languages are built in; use this only when a non-core language parser is missing. Available: go, php, c, cpp, c_sharp, java, ruby, dart.",
                        "inputSchema": {
                            "type": "object",
                            "properties": {
                                "action": {
